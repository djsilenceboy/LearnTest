...
public class InfoHelper
{
	...
	public static void showException( Logger log, String paddingSpace, Throwable t )
	{
		...
		else if (t instanceof AxisFault)
		{
			...
			log.error( paddingSpace + "failed: fault string: " + ((AxisFault)t).getFaultString() );

			Element [] eles = ((AxisFault)t).getFaultDetails();
			if (eles != null)
			{
				for (i = 0; i < eles.length; i++)
				{
					log.error( paddingSpace + "failed: fault details: [" + i + "]: " + eles[i].getNodeName() + " / " + eles[i].getTextContent() );
				}
			}
		}
		else

...
public class ItemDaoImplA implements ItemDao
{
	...
	public List<Item> query( String itemName )
	{
		...
		List<Item> items = jdbcTemplate.query( sql, new Object []
		{ itemName }, new RowMapper<Item>()
		{
			@Override
			public Item mapRow( ResultSet rs, int rowNum ) throws SQLException, DataAccessException
			{
				Item item = new Item();
